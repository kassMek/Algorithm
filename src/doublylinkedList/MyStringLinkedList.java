...
public class MyStringLinkedList {
	...
	public void insert3(String s, int pos) {
		if(pos < 0 || pos > size()) {
			throw new IndexOutOfBoundsException("pos = "+pos +" and size = "+size());
		}
		...
		if(previous.next == null) {
			previous.next = new Node(null, previous, s);
		}
		
		//otherwise, we are inserting between two nodes
		else {
			//make new node's next be previous.next; make new node's previous be previous
			Node newNode = new Node(previous.next, previous, s);
...
//		System.out.println(l);
//		System.out.println("Ordinary recursive search: contains Sally? " + l.recurSearch("Sally"));
//		System.out.println("Ordinary recursive search: contains Bob? " + l.recurSearch("Bob"));
//		
